...
struct tcp_header {
    ovs_be16 tcp_src;//源端口，16位
    ovs_be16 tcp_dst;//目的端口，16位
    ovs_16aligned_be32 tcp_seq;//发送数据包中的第一个字节的序列号，32位
    ovs_16aligned_be32 tcp_ack;//确认序列号，32位
    ovs_be16 tcp_ctl;//数据偏移和标志位
    ovs_be16 tcp_winsz;//表示接收缓冲区的空闲空间，16位，用来告诉TCP连接对端自己能够接收的最大数据长度。
    ovs_be16 tcp_csum;//校验和，16位
    ovs_be16 tcp_urg;//紧急指针，16位，只有URG标志位被设置时该字段才有意义，表示紧急数据相对序列号（Sequence Number字段的值）的偏移。
};
