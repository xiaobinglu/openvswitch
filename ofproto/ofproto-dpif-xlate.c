...
struct xlate_ctx {
    ...
    uint32_t orig_skb_priority; /* Priority when packet arrived. 包到达的优先级*/
    uint32_t sflow_n_outputs;   /* Number of output ports. 出口端口号数量*/
    odp_port_t sflow_odp_port;  /* Output port for composing sFlow action. */
    uint16_t user_cookie_offset;/* Used for user_action_cookie fixup. */
    bool exit;                  /* No further actions should be processed. 没有actions要处理了，退出*/

   /* These are used for non-bond recirculation.  The recirculation再循环 IDs are
    * stored in xout and must be associated with a datapath flow (ukey),
    * otherwise they will be freed when the xout is uninitialized.否则他们会被释放当xout初始化的时候
    *
    *
...
xlate_group_bucket(struct xlate_ctx *ctx, struct ofputil_bucket *bucket)
{
    ...
    /* Roll back flow to previous state.回滚到以前的状态
     * This is equivalent to cloning the packet for each bucket.等同于为每个bucket克隆包
     *
     ...
     * execution. MPLS是利用标记（label）进行数据转发的。当分组进入网络时，要为其分配固定长度的短的标记，并将标记与分组封装在一起，在整个转发过程中，交换节点仅根据标记进行转发。*/
    ctx->was_mpls = old_was_mpls;
